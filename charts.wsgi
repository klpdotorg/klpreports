...
class Charts:
    ...
    def GET(self, searchby, constid, rep_lang, rep_type,):
        try:
            ...
            data = {}
            util = CommonUtil()
            report_str = '-'.join([searchby, str(constid), rep_lang,
                                  rep_type])
            ...
            if rep_type.lower() == 'demographics':
                if data_json == '':
                    ...
                    data.update(util.countsTable(constype, [constid],
                                queries))
                    data.update(demographics.generateData(constype,
                                [constid]))
                    data.update(utils.DemographicsUtil.getDemographicsText(data,
                                lang, constype))
                ...
                if data_json == '':
                    ...
                    data.update(util.countsTable(constype, [constid],
                                queries))
                    data.update(finances.generateData(constype,
                                [constid]))
                    data.update(utils.FinancesUtil.getFinancesText(data,
                                lang, constype))
                    data_json = simplejson.dumps(data, sort_keys=True)
                    write_file(cache_file, data_json)
                web.header('Content-Type', 'text/html; charset=utf-8')
                ...
                if data_json == '':
                    ...
                    data.update(utils.InfraUtil.getInfraText(data,
                                lang, constype))
                    data_json = simplejson.dumps(data, sort_keys=True)
                    write_file(cache_file, data_json)
                web.header('Content-Type', 'text/html; charset=utf-8')
                ...
                if data_json == '':
                    ...
                    data.update(util.countsTable(constype, [constid],
                                queries))
                    data.update(library.generateData(constype,
                                [constid]))
                    data.update(utils.LibraryUtil.getLibText(data,
                                lang, constype))
                    data_json = simplejson.dumps(data, sort_keys=True)
                    write_file(cache_file, data_json)
                web.header('Content-Type', 'text/html; charset=utf-8')
                ...
                if data_json == '':
                    nutrition = Nutrition()
                    data.update(nutrition.generateData(constype,
                                [constid]))
                    data.update(utils.NutritionUtil.getNutriText(data,
                                lang))
                    data_json = simplejson.dumps(data, sort_keys=True)
                    write_file(cache_file, data_json)
                web.header('Content-Type', 'text/html; charset=utf-8')
                ...
                if data_json == '':
                    ...
                    data.update(util.countsTable(constype, [constid],
                                queries))
                    learning = Learning()
                    data.update(learning.generateData(constype,
                                [constid]))
                    data.update(utils.LearningUtil.getLearningText(data,
                                lang, constype))
                    data_json = simplejson.dumps(data, sort_keys=True)
                    write_file(cache_file, data_json)
                web.header('Content-Type', 'text/html; charset=utf-8')
                ...
                if data_json == '':
                    ...
                    data.update(utils.RTEUtil.getRTEText(data, lang,
                                constype))
                    data_json = simplejson.dumps(data, sort_keys=True)
